...
local function chatSuggestion(name, parameters, help)
  ...
  if parameters then
    for i = 1, #parameters do
      ...
      if argType and argType:sub(0, 1) == '?' then argType = argType:sub(2, #argType) end
      ...
      if parameters then
        for i = 1, #parameters do
          ...
          if argType then
            ...
            if argType == 'number' then value = tonumber(value) or value end

            local type = type(value)

            if type ~= argType and (not optional or type ~= 'nil') then
              local invalid = ('^1%s expected <%s> for argument %s (%s), received %s^0'):format(
                name,
                argType,
                i,
                arg,

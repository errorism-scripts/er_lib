...
local function eventTimer(event, delay)
  if delay and type(delay) == 'number' and delay > 0 then
    ...
    if (timers[event] or 0) > time then return false end
...
local function triggerServerCallback(_, event, delay, cb, ...)
  ...
  pendingCallbacks[key] = function(response, ...)
    ...
    if promise then return promise:resolve(response) end
...
lib.callback = setmetatable({}, {
  __call = function(_, event, delay, cb, ...)
    if not cb then
      ...
      if cbType == 'table' and getmetatable(cb)?.__call then cbType = 'function' end

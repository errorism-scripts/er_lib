...
function lib.dui:constructor(data)
  ...
  local dictName = ('er_lib_dui_dict_%s'):format(id)
  local txtName = ('er_lib_dui_txt_%s'):format(id)
  ...
  if self.private.debug then print(('Dui %s created'):format(id)) end
end
...
function lib.dui:remove()
  ...
  if self.private.debug then print(('Dui %s removed'):format(self.private.id)) end
end
...
function lib.dui:setUrl(url)
  ...
  if self.private.debug then print(('Dui %s url set to %s'):format(self.private.id, url)) end
end

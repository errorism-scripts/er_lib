...
function locale(str, ...)
  ...
  if lstr then
    if ... then return lstr and lstr:format(...) end
...
local function loadLocale(key)
  ...
  if not data then warn(('could not load \'locales/%s.json\''):format(key)) end
...
function lib.locale(key)
  ...
  if lang ~= 'en' then table.merge(locales, loadLocale(lang)) end
...
function lib.getLocale(resource, key)
  ...
  if locale then warn(('overwriting existing locale \'%s\' (%s)'):format(key, locale)) end

  locale = exports[resource]:getLocale(key)
  dict[key] = locale

  if not locale then warn(('no locale exists with key \'%s\' in resource \'%s\''):format(key, resource)) end
...
AddEventHandler('er_lib:setLocale', function(key)
  lib.locale(key)
end)

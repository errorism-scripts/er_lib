...
local service = GetConvar('er:logger', GetConvar('ox:logger', 'datadog'))
local buffer
local bufferSize = 0
...
local hostname = removeColorCodes(
  GetConvar('er:logger:hostname', GetConvar('ox:logger:hostname', GetConvar('sv_projectName', 'fxserver')))
)
...
if service == 'loki' then
  ...
  if lokiUser ~= '' then headers['Authorization'] = getAuthorizationHeader(lokiUser, lokiPassword) end

  if lokiTenant ~= '' then headers['X-Scope-OrgID'] = lokiTenant end

  if not lokiEndpoint:find(startingPattern) then lokiEndpoint = 'https://' .. lokiEndpoint end

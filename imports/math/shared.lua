...
local function parseNumber(input, min, max, round)
  ...
  if not n then error(('value cannot be converted into a number (received %s)'):format(input), 3) end

  n = round and math.floor(n + 0.5) or n

  if min and n < min then error(('value does not meet minimum value of \'%s\' (received %s)'):format(min, n), 3) end

  if max and n > max then error(('value exceeds maximum value of \'%s\' (received %s)'):format(max, n), 3) end
...
function math.tovector(input, min, max, round)
  ...
  if inputType == 'table' then
    ...
    if table.type(input) == 'array' then return vector(table.unpack(input)) end
...
function math.lerp(start, finish, duration)
  ...
  assert(
    ...
    ('expected argument 2 to have type \'%s\' (received %s)'):format(typeStart, typeFinish)
  )
  ...
  return function()
    ...
    if step < 1 then return interpFn(start, finish, step), step end

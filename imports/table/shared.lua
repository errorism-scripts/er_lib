...
local function table_matches(t1, t2)
  ...
  if tabletype1 ~= table.type(t2) or (tabletype1 == 'array' and #t1 ~= #t2) then return false end
...
local function table_deepclone(tbl)
  ...
  for k, v in pairs(tbl) do
    if type(v) == 'table' then tbl[k] = table_deepclone(v) end
  end
...
function rawset(tbl, index, value)
  if table.isfrozen(tbl) then frozenNewIndex(tbl) end

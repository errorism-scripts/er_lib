...
-- Some people have decided to load this file as part of er_lib's fxmanifest?
if resourceName == er_lib then return end

if lib and lib.name == er_lib then
  error(
    ('Cannot load er_lib more than once.\n\tRemove any duplicate entries from \'@%s/fxmanifest.lua\''):format(
      resourceName
    )
  )
end

local export = exports[er_lib]

if GetResourceState(er_lib) ~= 'started' then error('^1er_lib must be started before this resource.^0', 0) end
...
local function loadModule(self, module)
  ...
  local chunk = LoadResourceFile(er_lib, ('%s/%s.lua'):format(dir, context))
  local shared = LoadResourceFile(er_lib, ('%s/shared.lua'):format(dir))

  if shared then chunk = (chunk and ('%s\n%s'):format(shared, chunk)) or shared end

  if chunk then
    local fn, err = load(chunk, ('@@er_lib/imports/%s/%s.lua'):format(module, context))

    if not fn or err then
      if shared then
        lib.print.warn(
          ('An error occurred when importing \'@er_lib/imports/%s\'.\nThis is likely caused by improperly updating er_lib.\n%s\''):format(
            module,
            err
          )
        )
        fn, err = load(shared, ('@@er_lib/imports/%s/shared.lua'):format(module))
      end
...
function SetInterval(callback, interval, ...)
  ...
  if cbType ~= 'function' then return error(('Callback must be a function. Received %s'):format(cbType)) end
...
function ClearInterval(id)
  ...
  if not intervals[id] then return error(('No interval exists with id %s'):format(id)) end
...
local cache = setmetatable({ game = GetGameName(), resource = resourceName }, {
  __index = function(self, key)
    ...
    AddEventHandler(('er_lib:cache:%s'):format(key), function(value)
  ...
  __call = function(self, key, func, timeout)
    ...
    if value == nil then
      ...
      if timeout then SetTimeout(timeout, function()
        self[key] = nil
      end) end
    end
...
function lib.onCache(key, cb)
  if not cacheEvents[key] then getmetatable(cache).__index(cache, key) end
...
if context == 'client' then
  RegisterNetEvent(notifyEvent, function(data)
    if locale then
      if data.title then data.title = locale(data.title) or data.title end

      if data.description then data.description = locale(data.description) or data.description end
    end
...
for i = 1, GetNumResourceMetadata(cache.resource, 'er_lib') do
  local name = GetResourceMetadata(cache.resource, 'er_lib', i - 1)

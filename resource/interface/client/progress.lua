...
local function startProgress(data)
  ...
  while progress do
    if disable then
      ...
      if disable.sprint and not disable.move then DisableControlAction(0, controls.INPUT_SPRINT, true) end
    ...
    if interruptProgress(progress) then progress = false end
...
function lib.cancelProgress()
  if not progress then error 'No progress bar is active' end

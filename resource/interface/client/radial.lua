...
local function showRadial(id, option)
  ...
  if id and not radial then return error 'No radial menu with such id found.' end
...
function lib.registerRadial(radial)
  ...
  if currentRadial then refreshRadial(radial.id) end
end
...
function lib.addRadialItem(items)
  ...
  if isOpen and not currentRadial then refreshRadial() end
end
...
function lib.clearRadialItems()
  ...
  if isOpen then refreshRadial() end
end
...
RegisterNUICallback('radialClick', function(index, cb)
  ...
  if onSelect then
    if type(onSelect) == 'string' then return exports[menuResource][onSelect](0, currentMenu, itemIndex) end
...
RegisterNUICallback('radialBack', function(_, cb)
  ...
  if lastMenu.id then return showRadial(lastMenu.id, lastMenu.option) end
...
function lib.disableRadial(state)
  ...
  if isOpen and state then return lib.hideRadial() end
end
...
lib.addKeybind {
  ...
  onPressed = function()
    ...
    if isOpen then return lib.hideRadial() end
...
AddEventHandler('onClientResourceStop', function(resource)
  for i = #menuItems, 1, -1 do
    ...
    if item.resource == resource then table.remove(menuItems, i) end
  end
end)

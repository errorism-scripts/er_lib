...
local settings = {
  default_locale = GetConvar('er:locale', GetConvar('ox:locale', 'en')),
...
local userLocales = GetConvarInt('er:userLocales', GetConvarInt('ox:userLocales', 1)) == 1
...
RegisterCommand('er_lib', function()
  local inputSettings = {
    {
      type = 'checkbox',
  ...
  if userLocales then
    table.insert(inputSettings, {
      ...
      options = GlobalState['er_lib:locales'],

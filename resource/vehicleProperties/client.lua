...
RegisterNetEvent('er_lib:setVehicleProperties', function(netid, data)
  local timeout = 100
  while not NetworkDoesEntityExistWithNetworkId(netid) and timeout > 0 do
    Wait(0)
    timeout -= 1
  end
  if timeout > 0 then lib.setVehicleProperties(NetToVeh(netid), data) end
end)

AddStateBagChangeHandler('er_lib:setVehicleProperties', '', function(bagName, _, value)
  ...
  if NetworkGetEntityOwner(entity) == cache.playerId then
    lib.setVehicleProperties(entity, value)
    Entity(entity).state:set('er_lib:setVehicleProperties', nil, true)
  end
end)

...
local function closeCreator(cancel)
  if not cancel then
    if zoneType == 'poly' then points[#points + 1] = vec(xCoord, yCoord) end
    ...
    TriggerServerEvent('er_lib:saveZone', {
      zoneType = zoneType,
    ...
    if displayMode == 3 or displayMode == 4 then
      if alignMovementWithCamera then
        ...
        local forwardX, forwardY =
          getRelativePos(vec2(xCoord, yCoord), vec2(xCoord, yCoord + 2), GetGameplayCamRot(2).z)
    ...
    if controlsActive then
      ...
      elseif IsDisabledControlJustReleased(0, 32) then -- w
        ...
        if alignMovementWithCamera then
          ...
          if math.abs(newX) < minCheck then newX = 0.0 end

          if math.abs(newY or 0) < minCheck then newY = 0.0 end
          ...
          if math.abs(newValue) < minCheck then newValue = 0.0 end
      ...
      elseif IsDisabledControlJustReleased(0, 22) then -- space
        ...
        if zoneType == 'poly' then points[#points + 1] = vec2(xCoord, yCoord) end
...
CreateThread(function()
  ...
  TriggerEvent('chat:addSuggestion', '/zone', 'Starts creation of the specified zone', {
    ...
    {
      name = 'useLast',
      help = 'duplicates the last created zone of the specified type (box and sphere only, optional)',
    },
  })
end)
